...
<div class="card">
    <div class="card-header">
        <h3>Edit Officer Information</h3>
    </div>
    <div class="card-body">
        <form method="POST" action="{{ route('admin.officers.update', $officer) }}">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1rem;">
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $officer->name) }}" required
                ...
                <div>
                    ...
                    <input type="email" id="email" name="email" value="{{ old('email', $officer->email) }}" required
                           style="width: 100%; padding: 0.75rem; border: 2px solid #e1e5e9; border-radius: 5px;">
                    @error('email')
                        <div style="color: #e74c3c; font-size: 0.9rem; margin-top: 0.5rem;">{{ $message }}</div>
                    @enderror
            ...
            <div style="margin-bottom: 1rem;">
                ...
                <input type="text" id="xelenic_id" name="xelenic_id" value="{{ old('xelenic_id', $officer->xelenic_id) }}"
                       style="width: 100%; padding: 0.75rem; border: 2px solid #e1e5e9; border-radius: 5px;">
                @error('xelenic_id')
                    <div style="color: #e74c3c; font-size: 0.9rem; margin-top: 0.5rem;">{{ $message }}</div>
                @enderror
