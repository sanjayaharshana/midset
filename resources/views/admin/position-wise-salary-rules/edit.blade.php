...
@section('title', 'Edit Position Wise Salary Rule')
@section('page-title', 'Edit Position Wise Salary Rule')
...
<div class="card">
    <div class="card-header">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <h3>Edit Position Wise Salary Rule</h3>
    ...
    <div class="card-body">
        <form action="{{ route('admin.position-wise-salary-rules.update', $positionWiseSalaryRule) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 2rem;">
                <div>
                    ...
                    <select class="form-control @error('position_id') is-invalid @enderror" id="position_id" name="position_id" required>
                        ...
                        @foreach($positions as $position)
                            <option value="{{ $position->id }}" {{ old('position_id', $positionWiseSalaryRule->position_id) == $position->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select class="form-control @error('job_id') is-invalid @enderror" id="job_id" name="job_id">
                        ...
                        @foreach($jobs as $job)
                            <option value="{{ $job->id }}" {{ old('job_id', $positionWiseSalaryRule->job_id) == $job->id ? 'selected' : '' }}>
            ...
            <div style="margin-top: 1.5rem;">
                ...
                <input type="number" step="0.01" min="0" class="form-control @error('amount') is-invalid @enderror" id="amount" name="amount" value="{{ old('amount', $positionWiseSalaryRule->amount) }}" placeholder="Enter amount" required>
                @error('amount')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div style="margin-top: 1.5rem;">
                ...
                <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="3" placeholder="Enter description...">{{ old('description', $positionWiseSalaryRule->description) }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div style="margin-top: 1.5rem;">
                ...
                <select class="form-control @error('status') is-invalid @enderror" id="status" name="status" required>
                    ...
                    <option value="active" {{ old('status', $positionWiseSalaryRule->status) == 'active' ? 'selected' : '' }}>Active</option>
                    <option value="inactive" {{ old('status', $positionWiseSalaryRule->status) == 'inactive' ? 'selected' : '' }}>Inactive</option>
                </select>
                @error('status')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
