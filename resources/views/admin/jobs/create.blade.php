...
<div class="card">
    <div class="card-header">
        ...
        <p style="color: #6b7280; margin-top: 0.5rem;">Job number will be automatically generated when you select a client.</p>
    </div>
    <div class="card-body">
        <form action="{{ route('admin.jobs.store') }}" method="POST">
            ...
            <div class="form-group" style="margin-bottom: 1.5rem;">
                ...
                <textarea class="form-control @error('description') is-invalid @enderror" 
                          id="description" name="description" rows="4" placeholder="Describe the job details...">{{ old('description') }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-bottom: 1.5rem;">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control @error('reporter_officer_name') is-invalid @enderror" 
                           id="reporter_officer_name" name="reporter_officer_name" value="{{ old('reporter_officer_name') }}">
            ...
            <!-- Job Number Preview -->
            <div id="job-number-preview" style="background: #f8fafc; padding: 1rem; border-radius: 0.5rem; margin-bottom: 1.5rem; display: none;">
                <h4 style="margin-bottom: 0.5rem; color: #374151;">Generated Job Number:</h4>
                <div id="preview-number" style="font-family: monospace; font-size: 1.2rem; font-weight: bold; color: #1f2937;"></div>
                <small style="color: #6b7280;">Format: {year}/{client_short_code}/{sequence_number}</small>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const previewDiv = document.getElementById('job-number-preview');
    ...
    clientSelect.addEventListener('change', function() {
        if (this.value) {
            ...
            const clientCode = clientText.match(/\(([^)]+)\)/)[1];
            
            // For preview, we'll show a sample number
            previewNumber.textContent = `${year}/${clientCode}/001`;
        } else {
